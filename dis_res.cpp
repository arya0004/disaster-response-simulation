...
public:
    ...
    bool isTraversable() const {
        return !blocked && type != CellType::DEBRIS && fireIntensity < 60; 
    }
...
public:
    ...
    double calculateCost(const Position& agentPos, AgentType agentType) const {
        ...
        double urgencyFactor = (100 - urgency) * 0.05; 
        
        
        double efficiency = 1.0;
        switch(agentType) {
            case AgentType::FIRE_BRIGADE:
                efficiency = (type == TaskType::EXTINGUISH_FIRE) ? 0.1 : 3.0; 
                break;
            ...
            case AgentType::AMBULANCE:
                efficiency = (type == TaskType::TRANSPORT_TO_HOSPITAL) ? 0.1 : 3.0; 
                break;
        }
        
        return (distance * 0.7) + (urgencyFactor * 0.1) + (efficiency * 0.2); // Distance matters increase karo
    }
    ...
    string getDisplayName() const {
        switch(type) {
            ...
            case TaskType::RESCUE_VICTIM: return " VICTIM";
...
public:
    ...
    // Resource manage for all agents pani na ll
    bool hasSufficientResources() const {
        if (fuel < 50) return false; 
        switch(type) {
            case AgentType::FIRE_BRIGADE: return water > 25; 
...
private:
    ...
    static void giveConfigurationAdvice(const SimulationConfig& config) {
        ...
        if (fireToBrigadeRatio > 1.5) {
            cout << " High fire load! Consider more fire brigades.\n";
        }
        if (victimToRescueRatio > 3) {
            cout << " Many victims! Consider more rescue teams.\n";
        }
        if (victimToAmbulanceRatio > 4) {
            cout << "Transport bottleneck! Consider more ambulances.\n";
        }
...
public:
    ...
    void initializeGoals() {
        ...
        goalPlanner.addGoal(make_shared<Goal>("Rescue majority of victims", 
            [this]() { return rescuedVictims >= totalVictims * 0.7; }, 100)); //  70% needed
        ...
        goalPlanner.addGoal(make_shared<Goal>("Control fires", 
            [this]() { 
                ...
                return activeFires <= initialFires * 0.3;
            }, 80));
        
        goalPlanner.addGoal(make_shared<Goal>("Prevent too many deaths", 
            [this]() { return deceasedVictims <= totalVictims * 0.4; }, 90)); 
    ...
    void display() {
        ...
        cout << "         |  Waiting Victims: " << setw(3) << waitingVictims << "         ║\n";
...
int main() {
    try {
        ...
        cout << "  City Size: " << config.citySize << "x" << config.citySize << "\n";
        cout << " Agents: " << config.numRescueTeams << " Rescue, " 
             << config.numFireBrigades << " Fire Brigades, " << config.numAmbulances << " Ambulances\n";
        cout << " Initial Fires: " << config.numFires << " |  Initial Victims: " << config.numVictims << "\n";
